	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if !tt.expectError {
				...
				if !strings.HasSuffix(result, ".athena") {
					t.Errorf("GetDataDir() = %q, should end with '.athena'", result)
				}
...
func TestGetPidFilePath(t *testing.T) {
	...
	// Should contain .athena
	if !strings.Contains(result, ".athena") {
		t.Errorf("GetPidFilePath() = %q, should contain '.athena'", result)
	}
...
func TestGetLogFilePath(t *testing.T) {
	...
	// Should contain .athena
	if !strings.Contains(result, ".athena") {
		t.Errorf("GetLogFilePath() = %q, should contain '.athena'", result)
	}
...
func TestDataDirectoryIntegration(t *testing.T) {
	...
	expectedPidPath := filepath.Join(dataDir, "athena.pid")
	...
	expectedLogPath := filepath.Join(dataDir, "athena.log")
