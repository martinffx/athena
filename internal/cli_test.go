...
func TestApplyFlagOverrides(t *testing.T) {
	tests := []struct {
		...
		{
			name: "log format and file overrides",
			cfg: &config.Config{
				LogFormat: "text",
				LogFile:   "",
			},
			flags: map[string]string{
				"log-format": "json",
				"log-file":   "/var/log/athena.log",
			},
			expected: &config.Config{
				LogFormat: "json",
				LogFile:   "/var/log/athena.log",
			},
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if v, ok := tt.flags["log-format"]; ok {
				logFormat = v
			}
			if v, ok := tt.flags["log-file"]; ok {
				logFile = v
			}
			...
			if tt.cfg.LogFormat != tt.expected.LogFormat {
				t.Errorf("LogFormat: got %v, want %v", tt.cfg.LogFormat, tt.expected.LogFormat)
			}
			if tt.cfg.LogFile != tt.expected.LogFile {
				t.Errorf("LogFile: got %v, want %v", tt.cfg.LogFile, tt.expected.LogFile)
			}
		})
	}
}
...
func TestFlagPrecedence(t *testing.T) {
	...
	cfg, err := config.New(tmpfile.Name())
	if err != nil {
		t.Fatal(err)
	}
...
func TestBackwardCompatibility(t *testing.T) {
	...
	t.Run("default config loading", func(t *testing.T) {
		cfg, err := config.New("")
		if err != nil {
			t.Fatalf("Expected config to be loaded with empty path: %v", err)
		}
